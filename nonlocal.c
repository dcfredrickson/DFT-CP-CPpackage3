/*     nonlocal_byatom, part of the Fredrickson Group Chemical Pressure Package  
                  ...
                  Copyright (C) 2012-2025, by Daniel C. Fredrickson
...
/*  June 11, 2025:  Completed additions to support ABINIT Version 8 (and hopefully higher).  --DCF */
...
void read_psp_data(char * filename, struct psp_file * pspin) {
    FILE * f2;
...
void read_calc_nonlocal(char * filename, char * outfile, struct wfk_file * wfkin, struct psp_file * pspin, double occ_min, int dtset) {
    FILE * f2;
    FILE * f3;
    int k;
    char codvsn[110],str[110];
    char title[132];
    char test;
    int headform;
    int fform;
    ...
    printf("Abinit version:  %d\n",abinitversion);
 //   if(abinitversion > 7)  j = fread(str, sizeof(char), 2, f2);
    ...
    if(abinitversion > 7) {
       fread( & nshiftk_orig, sizeof(int), 1, f2);
       fread( & nshiftk, sizeof(int), 1, f2);
       fread( & mband, sizeof(int), 1, f2);
    }
    fread( & j, sizeof(int), 1, f2);
    fread( & j, sizeof(int), 1, f2);
    /*    int istwfk [KPOINTS_MAX]; */
	...
	for(k = 0; k< ntypat;k++) {
            fread( &amu[k],sizeof(double),1,f2);
        }
        fread( & j, sizeof(int), 1, f2);
        fread( & j, sizeof(int), 1, f2);
        fread( &kptopt, sizeof(int), 1, f2);
        fread( &pawcpxocc, sizeof(int), 1, f2);
        fread( &nelect, sizeof(double), 1, f2);
        fread( &cellcharge, sizeof(double), 1, f2);
        fread( &icoulomb, sizeof(int), 1, f2);
        fread( &kptrlatt_ax, sizeof(int), 1, f2);
        fread( &kptrlatt_ay, sizeof(int), 1, f2);
        fread( &kptrlatt_az, sizeof(int), 1, f2);
        fread( &kptrlatt_bx, sizeof(int), 1, f2);
        fread( &kptrlatt_by, sizeof(int), 1, f2);
        fread( &kptrlatt_bz, sizeof(int), 1, f2);
        fread( &kptrlatt_cx, sizeof(int), 1, f2);
        fread( &kptrlatt_cy, sizeof(int), 1, f2);
        fread( &kptrlatt_cz, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_ax, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_ay, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_az, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_bx, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_by, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_bz, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_cx, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_cy, sizeof(int), 1, f2);
        fread( &kptrlatt_orig_cz, sizeof(int), 1, f2);
        fread( &shiftk_orig_x, sizeof(double), 1, f2);
        fread( &shiftk_orig_y, sizeof(double), 1, f2);
        fread( &shiftk_orig_z, sizeof(double), 1, f2);
        fread( &shiftk_x, sizeof(double), 1, f2);
        fread( &shiftk_y, sizeof(double), 1, f2);
        fread( &shiftk_z, sizeof(double), 1, f2);
        fread( & j, sizeof(int), 1, f2);
    }
    for (k = 0; k < (npsp); k++) {
        fread( & j, sizeof(int), 1, f2);
        ...
        if(abinitversion > 7) fread(md5_pseudos,sizeof(char),32,f2); 
        fread( & j, sizeof(int), 1, f2);
    }
    if (abinitversion < 8) {
        fread( & j, sizeof(int), 1, f2);
...
void read_calc_den(char * filename, struct XSFfile * XSFIN) {
    FILE * f2;
    int k;
    char codvsn[110],str[110];
    char title[132];
    ...
    sscanf(codvsn,"%d.%s",&abinitversion,str);
    printf("\nAbinit version:  %d\n",abinitversion);
    if(abinitversion > 8)  j = fread(str, sizeof(char), 2, f2);
    fread( & headform, sizeof(int), 1, f2);
    fread( & fform, sizeof(int), 1, f2);
        printf("%s %d %d\n",codvsn,headform,fform);
    fread( & j, sizeof(int), 1, f2);
